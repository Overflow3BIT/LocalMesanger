...
class UDPClientTk:
    ...
    def auto_ping_loop(self):
        ...
        while self.running_ping and self.connected:
            ...
            if ping_time is not None:
                failed_pings = 0  # Сбрасываем счетчик неудачных пингов
                # Для авто-пинга не выводим сообщение о результате,
                # чтобы не засорять чат. Можно раскомментировать следующую строку:
                # self.queue_message('ping_result', f"Автопинг: {ping_time} мс")
            else:
                failed_pings += 1
                self.queue_message('error', f"Автопинг не удался ({failed_pings}/{max_failed_pings})")

                # Если слишком много неудачных пингов, отключаемся
